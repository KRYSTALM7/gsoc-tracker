...
export default function MentorsPage() {
  ...
  return (
    <AuthGuard>
      <Topbar />
      <style>{`
        .mentors-wrap { padding: 28px 32px; max-width: 1200px; margin: 0 auto; }
        .tpl-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 32px; }
        @media (max-width: 680px) {
          .mentors-wrap { padding: 16px; }
          .tpl-grid { grid-template-columns: 1fr; }
          .mentors-title { font-size: 22px !important; }
        }
      `}</style>
      ...
      <div className="mentors-wrap">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24, gap: 16, flexWrap: 'wrap' }}>
          <div>
            <div className="mentors-title" style={{ fontFamily: 'Fraunces, serif', fontSize: 28, fontWeight: 600, color: 'var(--text)', marginBottom: 4 }}>
          ...
          <button onClick={() => setForm({ ...BLANK, id: '' })} style={{ padding: '7px 16px', background: 'var(--accent2-dim)', border: '1px solid rgba(232,116,154,0.3)', borderRadius: 7, color: 'var(--accent2)', fontSize: 13, cursor: 'pointer', fontFamily: 'DM Sans', whiteSpace: 'nowrap' }}>
            + Log Interaction
          </button>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 32 }}>
          {mentors.length === 0
            ...
            : mentors.map(m => (
              <div key={m.id} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '16px 18px', display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--text3)', lineHeight: 1.5, marginBottom: 10 }}>{m.notes || m.topic || '—'}</div>
        ...
        <div className="tpl-grid">
          {[
            ...
            <div key={i} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: 16 }}>
              <div style={{ fontSize: 10, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text3)', marginBottom: 10 }}>{tpl.label}</div>
              ...
              <button onClick={() => navigator.clipboard.writeText(tpl.text).then(() => setToast('Copied!'))}
                style={{ marginTop: 12, background: 'var(--accent-dim)', border: 'none', borderRadius: 5, padding: '5px 14px', color: 'var(--accent)', fontSize: 11, cursor: 'pointer' }}>
                Copy
              </button>
            </div>
          ))}
      ...
      {/* Mentor Modal */}
      {form && (
        <div onClick={e => e.target === e.currentTarget && setForm(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(4px)', zIndex: 5000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border2)', borderRadius: 14, padding: 28, width: 500, maxWidth: '100%', maxHeight: '90vh', overflowY: 'auto', position: 'relative' }}>
            <div style={{ fontFamily: 'DM Sans', fontSize: 16, fontWeight: 700, color: 'var(--text)', marginBottom: 4 }}>{form.id ? 'Edit' : 'Log'} Interaction</div>
            ...
            {[
              { label: 'Mentor Name / Handle', key: 'name',  placeholder: '@mentor_handle' },
              { label: 'Topic / Summary',      key: 'topic', placeholder: 'What did you discuss?' },
            ].map(f => (
              <div key={f.key}>
                <label style={{ display: 'block', fontSize: 10, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text3)', marginBottom: 6, marginTop: 16 }}>{f.label}</label>
                <input value={(form as any)[f.key] || ''} onChange={e => set(f.key as keyof Mentor, e.target.value)} placeholder={f.placeholder} style={F} />
              </div>
            ))}

            <label style={{ display: 'block', fontSize: 10, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text3)', marginBottom: 6, marginTop: 16 }}>Notes</label>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
              {[
                ...
                <div key={s.key}>
                  <label style={{ display: 'block', fontSize: 10, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text3)', marginBottom: 6, marginTop: 16 }}>{s.label}</label>
            ...
            <label style={{ display: 'block', fontSize: 10, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text3)', marginBottom: 6, marginTop: 16 }}>Date</label>
